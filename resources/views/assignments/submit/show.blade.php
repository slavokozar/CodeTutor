...
@section('content')
    <section id="submit">
        ...
        @if(strtotime($assignmentObj->start_at) > time())
                        ...
                        <form id="submit-upload" method="post" enctype="multipart/form-data"
                            ...
                            @if($solutionObj != null)
                                <div id="submit-solution">
                                    <span id="submit-file" style="">{!! $solutionObj->icon() !!} {{$solutionObj->filename}}</span>
                                    <h4>Odovzdané súbory</h4>
                                    <div id="submit-files" class="minimized">
                                        {!! $files !!}
                                    </div>
                                    <button id="submit-files-minimize" class="hidden">
                                        <i class="fa fa-angle-double-up" aria-hidden="true"></i>
                                    </button>
                                    <button id="submit-files-maximize">
                            ...
                            @endif
                        <a href="{{action('Assignments\SubmitController@history', $assignmentObj->code)}}">História odovzdávania</a>


                        @if($solutionObj != null && $solutionObj->lang == '')
                            <div id="upload-error">
