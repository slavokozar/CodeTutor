    ...
    <div class="comment">
        ...
        @if(Auth::check() && $commentObj->canModify(Auth::user()))
            <div class="comment-tool">
                <a class="comment-remove" href="{{action('CommentController@destroy',[$objectObj->commentRoute(), $objectObj->code, $commentObj->id])}}">
                    <i class="fa fa-trash" aria-hidden="true"></i> Odstrániť

                </a>
                <a class="comment-edit" href="{{action('CommentController@edit',[$objectObj->commentRoute(), $objectObj->code, $commentObj->id])}}">
        ...
        @if(Auth::check() && $commentObj->canReply(Auth::user()))
            <a href="{{action('CommentController@create',[$objectObj->commentRoute(), $objectObj->code, $commentObj->reply_to_id == null ? $commentObj->id : $commentObj->reply_to_id])}}" class="comment-reply">
